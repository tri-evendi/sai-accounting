 ...
 * an icon AND a text label, so the meaning survives greyscale, colour-blind
 ...
 * `gap-1` dan `h-3 w-3` dulu ada karena `<svg>` di dalam badge tidak punya
 * ukuran maupun jarak. Keduanya sekarang milik `Tag` AntD sendiri:
 * `.ant-tag > svg + span` mendapat `margin-inline-start: paddingInline`, dan
 * ikon `@ant-design/icons` sudah `1em` tanpa disebut, sehingga ia mengikuti
 * `fontSizeSM` (12px) milik `Tag` — bukan angka tetap yang harus diubah kalau
 * kerapatan tema bergeser.
...
export async function VarianceBadge({
  ...
  const Icon = over ? ArrowUpOutlined : ArrowDownOutlined;
